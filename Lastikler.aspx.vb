...
Public Class Lastikler
    ...
    Public Sub DOSYA_GRIDLE_SQL()
        ...
        Dim CMD As New SqlCommand("SELECT * FROM SatisSistemi.DBO.PRODUCTS WHERE PRODUCT_TYPE='Lastik'", cn1)
    ...
    Public Sub DOSYA_GRIDLE_SQL_ara()
        Dim cn1 As New SqlConnection
        cn1.ConnectionString = WebConfigurationManager.ConnectionStrings("Con").ToString()
        Dim CMD As New SqlCommand("SELECT * FROM SatisSistemi.DBO.PRODUCTS WHERE PRODUCT_TYPE='Lastik' AND PRODUCT_NAME LIKE '%" + TxtAra.Text + "%'", cn1)
        CMD.CommandType = CommandType.Text
        Dim DA As New SqlDataAdapter(CMD)
        Dim DT As New DataTable
        DA.Fill(DT)
    ...
    Public Sub DOSYA_GRIDLE_MYSQL()
        ...
        Dim CMD As New MySqlCommand("SELECT * FROM PRODUCTS WHERE PRODUCT_TYPE='Lastik' ORDER BY PRODUCT_ID DESC", cn1)
    ...
    Public Sub DOSYA_GRIDLE_MYSQL_ara()
        Dim cn1 As New MySqlConnection
        cn1.ConnectionString = WebConfigurationManager.ConnectionStrings("Con_mysql").ToString()
        Dim CMD As New MySqlCommand("SELECT * FROM PRODUCTS WHERE PRODUCT_TYPE='Lastik' AND PRODUCT_NAME LIKE '%" + TxtAra.Text + "%'", cn1)
        CMD.CommandType = MySqlDbType.Text
        Dim DA As New MySqlDataAdapter(CMD)
        Dim DT As New DataTable
        DA.Fill(DT)
